...
- グラフまたは彩色の表現が曖昧でないこと
- 対象problemの禁止構造に対応する赤い `K_s` が存在しないこと
- 対象problemの禁止構造に対応する青い `K_t` が存在しないこと（グラフ表現では独立t集合）
...
## Stage 3 retrospective後の再現比較と統合後の記録

- 対象はproblem.mdに指定されたR(s,t)とし、テンプレートの旧R(3,4)用の条件を他benchmarkへ持ち込まない。
- 出力に環境・時刻等が含まれる場合、再実行前に数学的比較から除くキーを明示する。
  それ以外の全キーと値を比較し、raw出力・比較結果・環境の相違を保存する。
  数学的結果の一致とバイト一致を区別し、元のReviewer実行環境を再実行環境で書き換えない。
- 保存済みReviewer実装の再実行は再現確認であり、新規の独立実装・独立数学レビューとは別に記録する。
- Claim状態の4文書同期はmerge前に終える。merge SHA・日時の保存は統合担当の別工程とし、
  現在地の「統合待ち」を次の文書PR等で更新する。過去のレビューartifactを当時から統合済みだったように書き換えない。
